...
public final class Themes {
  ...
  public static void setWallpapers(HomeView parent, JMenu wallpaper) {
    IntStream.range(0, Themes.wallpapers.length).forEach(i -> {
      ...
      Themes.wallpapers[i].addActionListener(e -> paintBackground(e, parent));
      wallpaper.add(Themes.wallpapers[i]);
      wallpaper.addSeparator();
    });
  }

  public static void setThemes(HomeView parent, JMenu mode) {
    defaultMode = Resources.getMenuItem("Default", "default_theme");
    defaultMode.addActionListener(e -> {
      ...
      setUI(Themes.DEFAULT, parent);
    });

    darkMode = Resources.getMenuItem("Dark", "dark");
    darkMode.addActionListener(e -> {
      ...
      setUI(Themes.DARK, parent);
    });

    textureMode = Resources.getMenuItem("Texture", "texture");
    textureMode.addActionListener(e -> {
      if (Themes.textureMode.getForeground() == Styles.MAIN_COLOR) {
        Alerts.elementApplied(true);
        return;
      }

      setUI(Themes.TEXTURE, parent);
    });

    macMode = Resources.getMenuItem("Mac OS", "mac");
    macMode.addActionListener(e -> {
      if (Themes.macMode.getForeground() == Styles.MAIN_COLOR) {
        Alerts.elementApplied(true);
        return;
      }

      setUI(Themes.MAC, parent);
    });

    grayMode = Resources.getMenuItem("Metallic", "gray");
    grayMode.addActionListener(e -> {
      if (Themes.grayMode.getForeground() == Styles.MAIN_COLOR) {
        Alerts.elementApplied(true);
        return;
      }

      setUI(Themes.GRAY, parent);
    });

    mintMode = Resources.getMenuItem("Mint", "mint");
    mintMode.addActionListener(e -> {
      if (Themes.mintMode.getForeground() == Styles.MAIN_COLOR) {
        Alerts.elementApplied(true);
        return;
      }

      setUI(Themes.MINT, parent);
    });

    classicMode = Resources.getMenuItem("Classic", "classic");
    classicMode.addActionListener(e -> {
      ...
      setUI(Themes.CLASSIC, parent);
    });

    Resources.addMenu(
        mode, defaultMode, darkMode, textureMode, macMode, grayMode, mintMode, classicMode
    );
  }
