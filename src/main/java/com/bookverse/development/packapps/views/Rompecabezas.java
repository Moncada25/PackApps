    ...
    private void insertar(String state) {
        ...
        if (h.cr.comprobarConexion("Datos no guardados", true) && h.cr.saveGame()) {
            ...
            try {
                ...
                String data[] = {"rompecabezas", h.cr.ingreseNickname("Ingrese un Nickname", 20), state, getLevel(), String.valueOf(movimientos), h.cr.obtenerDate()};
    ...
    public void run() {
        ...
        try {
            ...
            while (cronometroActivo) {
                Thread.sleep(4); // recibe la cantidad de milisegundos de pausa que se har? cada que se ejecute
                // el hilo
            ...
            JOptionPane.showMessageDialog(null, "<html>" + h.cr.styleJOption() + "<strong>" + e.getMessage() + "</strong></html>");
        }
    }
}