...
public class AhorcadoTabla extends JDialog implements ActionListener, MouseListener {
    ...
    private void Componentes() {
        ...
        setIconImage(new ImageIcon(img.getImage("ahorcado.png")).getImage());
        ...
        ahorcadoTab = new JTable(modelo);
        ahorcadoTab.getTableHeader().setReorderingAllowed(false);
        scroll = new JScrollPane(ahorcadoTab);
        add(scroll, BorderLayout.CENTER);

        int[] anchos = {20, 170, 40, 20, 75, 100};
        for (int i = 0; i < ahorcadoTab.getColumnCount(); i++) {
            ahorcadoTab.getColumnModel().getColumn(i).setPreferredWidth(anchos[i]);
        }
        ...
        ahorcadoTab.setRowSorter(ordenar);

        for (int i = 0; i < columnas.length; i++) {
            DefaultTableCellRenderer tcr = new DefaultTableCellRenderer();
            tcr.setHorizontalAlignment(SwingConstants.CENTER);
            ahorcadoTab.getColumnModel().getColumn(i).setCellRenderer(tcr);
        }
            ...
            if (opcion != null) {
                ...
                try {
                    ...
                    if (opcion.toString().equals("ID")) {
                        String sql = "select * from ahorcado where ID ='" + img.cr.ingreseNumero("Ingresa un ID", 6)
                                + "'";
                        resultadoBusqueda(90, sql);
                        setVisible(true);
                    } else if (opcion.toString().equals("Nickname")) {
                        String sql = "select * from ahorcado where Nickname ='"
    ...
    private void btnUpdateAP() {
        ...
        if (ahorcadoTab.getRowCount() != 0) {
            ...
            int filaseleccionada = ahorcadoTab.getSelectedRow();
                ...
                if (img.cr.loginDBA()) {
                    try {
                        img.db.updateData(img.cr.ingreseNickname("Ingrese un Nickname", 20),
                                String.valueOf(modelo.getValueAt(filaseleccionada, 0)), "ahorcado");
    ...
    private void btnDeteleAP() {
        ...
        if (ahorcadoTab.getRowCount() != 0) {
            ...
            if (ahorcadoTab.getSelectedRow() == -1) {
                img.cr.mostrarMensaje("Delete", "No se ha seleccionado ningún registro.");
            } else {

                int[] rows = ahorcadoTab.getSelectedRows();
                ...
                if (img.cr.loginDBA()) {
                    try {
                        img.db.deleteData(IDs, "ahorcado");
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }
